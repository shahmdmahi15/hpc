...
interface HandlerHeaderProps {
  connectionStatus: ConnectionStatus;
  currentUserRole?: Role;
}

export function HandlerHeader({
  connectionStatus,
  currentUserRole,
}: HandlerHeaderProps) {
  ...
  return (
    <header className="w-full px-3 sm:px-6 py-1.5 border-b border-border/70 bg-card/85 backdrop-blur-xl sticky top-0 z-30 flex items-center justify-between gap-2.5 shadow-xs">
      {/* 1. Left: Brand & Physical Therapy Desk Identity */}
      <div className="flex items-center gap-2.5 shrink-0">
        <BrandLogo size="sm" variant="full" />
        <div className="hidden md:flex items-center gap-1.5 pl-2.5 border-l border-border/60">
          <span className="inline-flex items-center gap-1.5 px-2 py-0.5 rounded-md bg-emerald-500/10 border border-emerald-500/20 text-emerald-700 dark:text-emerald-300 text-[10.5px] font-bold tracking-wide uppercase">
            <Activity className="size-3" />
            <span>Physical Therapy Desk</span>
          </span>
        </div>
      </div>
      ...
      <div className="hidden lg:flex items-center gap-3">
        ...
        <div className="flex items-center gap-1.5 px-2.5 py-0.5 rounded-lg bg-muted/40 border border-border/60 text-xs font-mono">
          <Clock className="size-3 text-emerald-500 animate-pulse" />
