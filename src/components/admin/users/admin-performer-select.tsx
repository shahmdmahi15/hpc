...
export function AdminPerformerSelect({
  ...
  // If exactly 1 admin performer exists and none is selected, auto-select it
  React.useEffect(() => {
    if (adminPerformers.length === 1 && !selectedPerformerId) {
      onSelectPerformerId(adminPerformers[0].id);
    }
  }, [adminPerformers, selectedPerformerId, onSelectPerformerId]);

  // Case 1: No admin performers registered yet (Root Administrator fallback)
        ...
        <div className="flex items-center gap-2">
          ...
          <div>
            <span className="font-semibold text-foreground">
              Acting as Root Administrator
            </span>
            <p className="text-[10.5px] text-muted-foreground">
              No individual admin staff profiles registered yet.
            </p>
          </div>
        </div>
  ...
  // Case 2: Exactly 1 admin performer registered
  if (adminPerformers.length === 1) {
    const single = adminPerformers[0];
    return (
  ...
  // Case 3: Multiple admin staff exist — Selection is strictly mandatory!
  return (
    <div className="space-y-2">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 max-h-40 overflow-y-auto pr-1">
        {adminPerformers.map((admin) => {
          const isSelected = admin.id === selectedPerformerId;
          ...
          return (
            <button
              ...
              type="button"
              disabled={disabled}
              onClick={() => onSelectPerformerId(admin.id)}
      ...
      {error ? (
        ...
        <p className="text-[11px] text-amber-600 dark:text-amber-400 font-medium flex items-center gap-1">
          ...
          Please select which administrator is performing this action.
        </p>
      ) : null}
    </div>
  );
